...
import { Gift, CreditCard, Check, ArrowLeft, Sparkles, Heart, Mail, User, Phone, MessageSquare, DollarSign, Lock, Shield, Star, Loader2 } from 'lucide-react'
...
export default function PurchaseGiftCard() {
    ...
    const fetchStore = async () => {
      try {
        ...
        setStore({
          ...
          giftCardsEnabled: data.store.giftCardsEnabled ?? true,
        })
      } catch (error) {
  ...
  if (!store.giftCardsEnabled) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-white">
        <div className="text-center max-w-md mx-auto px-4">
          <Gift className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Gift Cards Not Available</h1>
          <p className="text-slate-600">Sorry, {store.name} does not currently offer gift cards.</p>
          <button 
            onClick={() => router.push(`/store/${slug}`)} 
            className="mt-6 px-6 py-3 bg-[rgb(var(--color-primary))] text-white rounded-xl font-medium"
          >
            Back to Menu
          </button>
        </div>
                ...
                <div className="flex items-end justify-between">
                  <div>
                    ...
                    <p className="text-4xl sm:text-5xl font-bold">
                      ${purchasedCard.currentBalance?.toFixed(2) || purchasedCard.amount?.toFixed(2)}
                    </p>
                  </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              ...
              <p className="text-lg text-white/80 max-w-md">
                Give the gift of great food. Perfect for any occasion.
              </p>
            </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isGift 
                      ? "Happy Birthday! Enjoy some delicious food on me! 🎉"
